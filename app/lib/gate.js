/** Relative path opened when an organizer scans a fan's ticket QR. */
...
/** Compact payload for offline/debug scanners (token + serial). */
...
export function parseGateScanPayload(text) {
  ...
  try {
    ...
    if (data?.tokenId && data?.serial != null) {
      return { tokenId: String(data.tokenId), serial: Number(data.serial) };
    }
  } catch {
    /* not JSON — try URL */
  }
  try {
    ...
    if (match) {
      return { tokenId: decodeURIComponent(match[1]), serial: Number(match[2]) };
    }
